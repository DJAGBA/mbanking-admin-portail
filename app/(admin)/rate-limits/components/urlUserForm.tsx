...
// Component for the multi-step wizard to assign or edit a URL limit for a user, with user selection, quota configuration, and summary steps
const USERS_PER_PAGE = 5;
export function AssignUrlUserStepper({ user, onSubmit, onCancel }: any) {
  const [step, setStep] = useState(1);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [users, setUsers] = useState<UserData[]>([]);
  const [usersLoading, setUsersLoading] = useState(true);
  const [selectedUser, setSelectedUser] = useState<UserData | null>(user || null);
...
// Filter users based on search term and paginate the results for the user selection step
  const filteredUsers = useMemo(() =>
    users.filter(u =>
      u.username.toLowerCase().includes(searchTerm.toLowerCase()) ||
      u.email.toLowerCase().includes(searchTerm.toLowerCase())
    ), [users, searchTerm]);
// Calculate total pages and slice the filtered users for pagination
  ...
  // Handle changes in the form inputs for quota configuration
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    setFormData(prev => ({ ...prev, [name]: value }));
  };

  const isQuotaStep = (!user && step === 2) || (user && step === 1);
  const isLastStep = (!user && step === 3) || (user && step === 2);
...
// Handle form submission by validating the input and calling the onSubmit prop with the selected user ID and configured quotas
  const handleSubmit = async () => {
    ...
    try {
      ...
      await onSubmit(String(selectedUser!.id), payload);
    } catch (err: unknown) {
        ...
        <div className="p-6 bg-gray-50 border-t flex gap-4 shrink-0">
          ...
          {!isLastStep ? (
            <button
              type="button"
              onClick={nextStep}
              className="flex-[2] px-8 py-3 bg-primary hover:bg-primary/90 text-white font-semibold rounded-lg transition-all flex items-center justify-center gap-2"
            >
            ...
            <button
              ...
              disabled={loading}
              className="flex-[2] px-8 py-3 bg-primary hover:bg-primary/90 text-white font-semibold rounded-lg transition-all disabled:opacity-50"
            >
